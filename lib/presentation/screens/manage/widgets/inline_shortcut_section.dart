...
class InlineShortcutSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final shortcutsAsync =
        ref.watch(shortcutsByCategoryProvider(categoryId));
            ...
            Padding(
              ...
              child: Column(
                children: shortcuts
                    .map((s) => _ShortcutRow(
                          shortcut: s,
                          onEdit: () => _showShortcutDialog(
                              context, ref, categoryId, s),
                          onDelete: () =>
    ...
    await showDialog<void>(
      ...
      builder: (_) => ShortcutDialog(
        ...
        onSave: (companion) async {
          ...
          if (existing == null) {
            final current =
                ref.read(shortcutsByCategoryProvider(categoryId)).valueOrNull ??
                    [];
...
class _ShortcutRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ShortcutListTile(
      ...
      onDelete: onDelete,
      onLaunch: () async {
        final result = await launcher.launch(shortcut);
